...
pub async fn create_contract(
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let create_contract_response: crate::models::CreateContractResponse =
            serde_json::from_str(&local_var_content).map_err(Error::from)?;
        Ok((create_contract_response, headers))
    } else {
...
) -> Result<(crate::models::PostContractSourceResponse, HeaderMap), Error<CreateContractSourcesError>>
{
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let create_sources_parsed = serde_json::from_str(&local_var_content).map_err(Error::from);
        Ok((create_sources_parsed?, headers))
    } else {
...
pub async fn get_contracts(
    ...
    let local_var_status = local_var_resp.status();
    let header = local_var_resp.headers().to_owned();
    let local_var_content = local_var_resp.text().await?;

    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let parsed_content: crate::models::GetContractsResponse =
            serde_json::from_str(&local_var_content).map_err(Error::from)?;
        Ok((parsed_content, header))
    } else {
